class Employers::AppliesController < Employers::EmployersController
  ...
  before_action :before_hanlding_apply_undefined_job, only: %i(show update), if: -> {@apply.job.exception?}
  ...
  def update_apply_undefined_job
    job_ids = params[:job_ids]
